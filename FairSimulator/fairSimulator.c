...
int main(){
    ...
    guestType guests[MAX_GUESTS];
    guestType *guestPtr = guests;
    ...
    //Assining tickets and passes to the ticketbooth
    ...
    for(int fillGuest = 0; fillGuest < MAX_GUESTS; fillGuest++){
        ...
        int randomAge = randomNum(1,5); //returns a number between 1-4
        if (randomAge ==1 || randomAge == 2){ //Doubling the chances of the guest being a teenager
            guests[fillGuest].age = randomNum(13,20);
        }else if(randomAge == 3){
            guests[fillGuest].age = randomNum(5,13);    
        }else {
            guests[fillGuest].age = randomNum(20,66);
    ...
    for(int heightFill = 0; heightFill < MAX_GUESTS; heightFill++){
        if(guests[heightFill].age <= PRE_TEEN){
            heightSum = randomNum(72,82);
            guests[heightFill].guestHeight = (guests[heightFill].age * 6) + heightSum;
        }else{
            guests[heightFill].guestHeight = randomNum(149,281);
        }
    }
    ...
    for(int moneyFill =0; moneyFill < MAX_GUESTS; moneyFill++){
        ...
        decade = guests[moneyFill].age / DECADE;
        //assigning money based of decade and randomization
        totalWallet = (decade)*((randomNum(0,6) *5) + 20) + minMoney;
        guests[moneyFill].guestMoney = totalWallet;
        //initilizing values
        guests[moneyFill].guestTickets = 0;
        guests[moneyFill].numOfRides =0;
    ...
    for (int i =0; i< MAX_GUESTS; i++){
        ...
        if(admitGuest(fairPtr,guestPtr) == 1 ){ 
            //Admitting the guest
            numOfTickets = randomTicket(10,6,5); 
              
           if(randomer == 0){
               //if purchasing ride ticket fails use the forepurchase function
               if (purchaseRideTickets(fairPtr,guestPtr,numOfTickets) == 1 ){
                }else{
                    forcePurchase(fairPtr,guestPtr,numOfTickets);
                }
           }else{
               //Buying day pass
               if(purschaseDayPass(fairPtr,guestPtr) ==1){
               }else{ 
                   forcePurchase(fairPtr,guestPtr,MAX_SINGLE_TICKETS);
               }
           }
        }
        guestPtr++;
    ...
    for (int j =0; j< RIDE_ATTEMPS; j++ ){
        for (int riding = 0; riding < MAX_GUESTS; riding ++){
             goingOnRide(guestPtr,&fairRides.rides[0]);
             guestPtr++;
        }
        //resetting the pointer
        guestPtr -= MAX_GUESTS;   
    ...
    //printing stats
     ...
     for (int guester =0; guester <MAX_GUESTS; guester++){
         ...
         if(guestPtr->hasAllDayPass ==1){
             printf("Guest #%3d, Age: %2d, Height: %2dcm, Money: %6.2f,  Tickets:PASS, ", guester,guestPtr->age,guestPtr->guestHeight,  guestPtr->guestMoney);
         }else{  
          printf("Guest #%3d, Age: %2d, Height: %3dcm, Money: %6.2f,  Tickets: %3d, ", guester,guestPtr->age,guestPtr->guestHeight,  guestPtr->guestMoney, guestPtr->guestTickets);
         }
         printf("Rides: %2d \n",guestPtr->numOfRides);
         guestPtr++;
     }
...
/*
   Function: This function is called when buying a ticket is failed, this is most likely the cause of my profits being high 
   //The ticket value goes down by 1 till the guests can afford the tickets
